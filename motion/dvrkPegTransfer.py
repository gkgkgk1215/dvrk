...
class dvrkPegTransfer(threading.Thread):
    """
    ...
    def __init__(self, interval_ms=10.0, ros_namespace='/dvrk'):
        ...
        self.arm = dvrkArm1('/PSM2')
        self.arm = dvrkArm2('/PSM2')
        self.t = 0.0
        self.nStart = 0.0
        self.nEnd = 0.0
    ...
    """
    Motion Creating function for peg transfer task
    """
    ...
    def pickup(self, pos_pick, rot_pick, height_pick, unit='rad'):
        # move upon the pick-up spot and open the jaw
        p_temp = np.array([pos_pick[0], pos_pick[1], height_pick])
        if unit=='deg':
            rot_pick =
