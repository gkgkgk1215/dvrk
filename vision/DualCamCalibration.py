...
class ImageConverter():
    def __init__(self):
        ...
        # threading
        t1 = threading.Thread(target=self.__run, args=(lambda: self.__stop_flag,))  # mainthread
        ...
        # ROS subscriber
        self.__sub = rospy.Subscriber('/kinect2/hd/image_color/compressed', CompressedImage, self.__img_raw_cam2_cb)
    ...
    def __run(self, stop):
        while True:
            if self.__img_raw_cam1==[] or self.__img_raw_cam2==[]:
                ...
                img2 = cv2.resize(self.__img_raw_cam2, (640, 360))  # AuxCam (Kinect2)

                # Stack both images horizontally
                # images = np.hstack((img1, img2))

                cv2.imshow("Main cam", img1)
                cv2.imshow("Aux cam", img2)
